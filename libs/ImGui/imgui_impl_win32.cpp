...
static void ImGui_ImplWin32_UpdateGamepads()
{
    ...
    //  Some players may not want to utilize the controller inputs to navigate the menu
    //  An alternative to this would be remove the XINPUT_GAMEPAD_X + XINPUT_GAMEPAD_THUMBL menu movement instead
    //switch (SelectedGame) {
    //case(1):
    //    if (!SOCOM2::g_GameVariables->m_CONTROLLER)
    //        return;
    //    break;
    //case(3):
    //    if (!SOCOM_CA::g_GameVariables->m_CONTROLLER)
    //        return;
    //    break;
    //}
    ...
    if (g_HasGamepad && g_XInputGetState && g_XInputGetState(0, &xinput_state) == ERROR_SUCCESS)
    {
        ...
        /// SHOW / HIDE MENU (CONTROLLER)
        //  - info
        //  DPAD_DOWN + BUTTON_B = 0x2002   (Old button combo)
        //  L3 + R3 = 0xC0  (Current button combo)
        ...
        ///  MAGIC BULLET HUD CHECK
        // We do not want to pass our button inputs to ImGui if the only ImGui window showing is part of our Notification Class
        // we place this below our input listener so that we can show  / hide the menu regardless of this state.
        //switch (SelectedGame) {
        //case(3):
        //    if (SOCOM_CA::g_GameData->bDIST_SILENTAIM && !SOCOM_CA::g_GameVariables->m_ShowMenu)
        //        return;
        //    break;
        //};
